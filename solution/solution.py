...
class QAPair:
    ...
    question: str
    expected_answer: str
    context: str | list = ""                    # Hoặc contexts
    id: str = "test_id"                         # Gán giá trị mặc định để tránh lỗi missing arg
    difficulty: str = "easy"                    # Gán giá trị mặc định để tránh lỗi missing arg
...
class RAGASEvaluator:
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        completeness = len(intersection) / len(expected_tokens)
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        # Tập hợp hợp (Union) tất cả các tokens từ mọi chunk trong contexts
        union_tokens = set()
        for chunk in contexts:
            union_tokens.update(_tokenize(chunk))

        intersection = expected_tokens.intersection(union_tokens)
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            coverage = len(chunk_tokens.intersection(expected_tokens)) / len(expected_tokens)
            rel_flags.append(coverage >= relevance_threshold)

        total_relevant = sum(rel_flags)
        if total_relevant == 0:
            return 0.0

        # 2 & 3. Tính Rank-Aware Average Precision (AP@K)
        precisions = []
    ...
    def run_full_eval(
        ...
        """
        # 1. Tính 3 answer-side metrics bắt buộc
        faithfulness = self.evaluate_faithfulness(answer, context)
        relevance = self.evaluate_relevance(answer, question)
        completeness = self.evaluate_completeness(answer, expected)

        # 2. Xác định trạng thái passed (đạt nếu cả 3 scores >= 0.5)
        passed = faithfulness >= 0.5 and relevance >= 0.5 and completeness >= 0.5

        # 3. Phân loại lỗi failure_type theo đúng thứ tự ưu tiên (first match wins)
        failure_type: str | None = None
        if not passed:
            if faithfulness < 0.3:
        ...
        # 4. Tính toán retrieval-side metrics nếu contexts được truyền vào
        context_recall: float | None = None
        context_precision: float | None = None
        ...
        # 5. Tạo đối tượng QAPair tạm thời (Sửa 'contexts' -> 'context')
        dummy_qa = QAPair(
            id="",
            difficulty="",
            question=question,
            expected_answer=expected,
            context=[{"source_doc": "", "text": context}], # <-- Sửa tại đây (context thay vì contexts)
            retrieved_contexts=contexts,
        )

        # 6. Trả về kết quả EvalResult đầy đủ
        return EvalResult(
            qa_pair=dummy_qa,
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    return sorted(
        contexts,
        key=lambda c: len(set(_tokenize(c)).intersection(query_tokens)),
        reverse=True
    )
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        # 1. Format criteria for the prompt
        criteria_str = "\n".join(
            [f"- {name}: {desc}" for name, desc in rubric.items()]
        )

        prompt = f"""You are an expert evaluator assessing an AI response.
...
Evaluate the response against each criterion on a scale from 0.0 to 1.0.
Output your evaluation strictly in valid JSON format structured like this:
{{
  "scores": {{
...
"""
        ...
        # Default fallback structure in case parsing fails
        default_scores = {criterion: 0.5 for criterion in rubric}
        default_result = {
            "scores": default_scores,
            "reasoning": raw_response or "Failed to parse judge output.",
        }

        if not raw_response or not isinstance(raw_response, str):
            return default_result

        # 3. Parse JSON from response (handles potential markdown ```json blocks)
        try:
            ...
            # Validate that returned criteria match rubric requirements and are floats
            validated_scores = {}
            for criterion in rubric:
                val = scores.get(criterion)
                if isinstance(val, (int, float)):
                    # Clamp values strictly between 0.0 and 1.0
                    validated_scores[criterion] = float(max(0.0, min(1.0, val)))
                else:
                    # Fallback for missing/invalid criterion score
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Calculate overall mean across all evaluated scores
        overall_avg = sum(all_scores) / len(all_scores)

        leniency_bias = overall_avg > 0.8
        severity_bias = overall_avg < 0.3

        # Positional bias check: compares average score of the 1st entry against the rest
        positional_bias = False
        if len(scores_batch) > 1:
            first_entry_scores = list(scores_batch[0].get("scores", {}).values())
            rest_entry_scores = [
                s
            ...
            if first_entry_scores and rest_entry_scores:
                avg_first = sum(first_entry_scores) / len(first_entry_scores)
                avg_rest = sum(rest_entry_scores) / len(rest_entry_scores)
                
                # Triggers if the first item scores higher than the remainder overall
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # 2. Tổng hợp ngữ cảnh từ pair.context (hoặc pair.contexts)
            # Chú ý: Đề phòng trường hợp field tên là context hay contexts
            context_list = getattr(pair, "context", None) or getattr(pair, "contexts", [])
            
            # Ghép chuỗi các text trong context thành 1 đoạn duy nhất
            if isinstance(context_list, list):
                combined_context = " ".join(
                    [item.get("text", "") if isinstance(item, dict) else str(item) for item in context_list]
                )
            else:
                combined_context = str(context_list)

            # 3. Lấy retrieved_contexts nếu có
            retrieved_contexts = getattr(pair, "retrieved_contexts", None)
            ...
            # 5. Gán lại qa_pair gốc cho EvalResult đúng như yêu cầu đề bài
            eval_res.qa_pair = pair
            eval_res.actual_answer = actual_answer
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        # Tính trung bình cho Retrieval metrics (chỉ lấy các giá trị không None)
        ...
        # Đếm số lượng loại lỗi failure_types
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict[str, Any]:
        ...
        """
        def get_averages(res_list: list[EvalResult]) -> tuple[float, float, float]:
            if not res_list:
                return 0.0, 0.0, 0.0
            n = len(res_list)
            f = sum(r.faithfulness for r in res_list) / n
            r = sum(r.relevance for r in res_list) / n
            c = sum(r.completeness for r in res_list) / n
            return f, r, c
        ...
        regressions = []
        # Điểm giảm lớn hơn 0.05 so với baseline được coi là Regression
    ...
    def identify_failures(
        ...
        failures = []
        for r in results:
            # Lấy tất cả các scores có giá trị float (bỏ qua các giá trị None của retrieval nếu không có)
            ...
            # Nếu có bất kỳ chỉ số nào nhỏ hơn threshold
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # Kiểm tra xem có nhiều hơn 1 chỉ số cùng thấp bằng min_score và < 0.5 không
        lowest_metrics = [k for k, v in scores.items() if v == min_score and v < 0.5]

        if len(lowest_metrics) > 1:
            return "Multiple issues detected — review full pipeline"

        if min_score == scores["faithfulness"]:
            return "Context is missing or irrelevant — improve retrieval"
        elif min_score == scores["relevance"]:
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures, start=1):
            ...
            # Đảm bảo không bị IndexError nếu độ dài danh sách suggestions ngắn hơn failures
            fix = suggestions[i - 1] if i - 1 < len(suggestions) else "N/A"
            status = "Open"

            table_lines.append(f"| {failure_id} | {f_type} | {root_cause} | {fix} | {status} |")
    ...
    def generate_improvement_suggestions(
        ...
        # Đưa ra gợi ý theo tần suất lỗi phổ biến nhất
        ...
        # Bổ sung các gợi ý mặc định nếu chưa đủ 3 suggestions
        default_suggestions = [
